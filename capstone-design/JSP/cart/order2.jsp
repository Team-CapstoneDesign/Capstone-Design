...
<body>
 ...
 %> <img src="../../images/recycle/recycle_rq.png"> 캡슐 재활용 백 수거
					신청 완료. <%
 	} else {
 %> <img src="../../images/recycle/recycle_bag.png"> 캡슐 재활용 백
					수거 미신청. <%
 	}
 %> <input type="hidden" name="recycle" value="<%=rec%>">
			...
			<div class="cart_Info">
				<table>
					...
					<tr>
						<th>받는 사람<br> <input type="hidden" name="oName"
							value="<%=oName%>"><%=oName%>
						</th>
						<th>번호<br> <input type="hidden" name="otel"
							value="<%=otel%>"><%=otel%>
						</th>
						<th>주소<br> <input type="hidden" name="oZipno"
							value="<%=oZipno%>"> <%=oZipno%> (우편번호) <br> <input
							type="hidden" name="oAddress1" value="<%=oAddress1%>"> <%=oAddress1%>
							(기본주소) <input type="hidden" name="oAddrplus"
							value="<%=oAddrplus%>"> <%=oAddrplus%> (참고주소)<br> <input
							type="hidden" name="oAddress2" value="<%=oAddress2%>"> <%=oAddress2%>
							(상세 주소)
						</th>
					</tr>
					<tr class="cart_payInfo">
						<th><span>결제 수단</span><br>
						<input type="hidden" name="payment" value="<%=pay%>"><%=pay%></th>
					</tr>
				</table>
				...
				<div>
					<table class="cartTotal_box_final">
						...
						<tr class="cartTotal_box_number_final">
							...
							<th><span class="realTotal"><input type="hidden"
									name="price" value="<%=total%>"><%=String.format("%,d", total)%>
							</span>원</th>
						</tr>
					</table>
				</div>
			...
			<div class="cart_Info_button">
				...
				<input type="button" class="btn_nextPage" value="주문하기  >"
					onclick="orderOK()"></a>
			</div>
